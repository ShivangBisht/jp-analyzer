...
class StartupConfig:
    ...
    def diagnostics(self):
        return {"configPath": str(self.config_path) if self.config_path else None, "analyzerRepository": str(self.analyzer_repo), "analyzerPython": str(self.analyzer_python), "frontendRepository": str(self.frontend_repo), "frontendCommand": list(self.frontend_command), "kwjaExecutable": str(self.kwja_executable) if self.kwja_executable else None, "analyzerUrl": self.analyzer_url, "frontendUrl": self.frontend_url, "resolutions": self.resolutions}
...
def load_config(analyzer_repo: Path, local_path: Path | None = None):
    ...
    return StartupConfig(analyzer_repo, _required(py, "JP Analyzer Python"), _required(fe, "Novel Audio Miner repository"), command, analyzer.get("host", "127.0.0.1"), int(analyzer.get("port", 8766)), frontend.get("host", "127.0.0.1"), int(frontend.get("port", 5173)), kw.value, bool(startup.get("openBrowser", True)), float(startup.get("analyzerTimeoutSeconds", 120)), float(startup.get("frontendTimeoutSeconds", 60)), path if path.exists() else None, resolutions)
